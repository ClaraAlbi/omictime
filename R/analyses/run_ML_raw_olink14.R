...
data_all <- readRDS("raw_olink_14panels.rds")
...
system.time({
  for (cv in 1:5) {
    ...
    lightgbm::lgb.save(lgb_model, glue("cv.{type}_lightgbm_cv{cv}.lgb"))
    ...
    xgboost::xgb.save(xgb_model, glue("cv.{type}_xgb_cv{cv}.xgb"))
