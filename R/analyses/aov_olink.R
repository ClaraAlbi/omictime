...
olink_times <- data.table::fread("/mnt/project/olink_processing_start_date.dat.txt")
batch <- data.table::fread("/mnt/project/olink_batch_number.dat.txt")
panel <- data.table::fread("/mnt/project/olink_assay.dat") %>%
...
prots <- data.table::fread("/mnt/project/olink_instance_0.csv") %>%
  select(-glipr1)


for (b in seq(2, ncol(prots), 100)){
  #for (b in seq(1102, ncol(prots), 100)) {
  l1 <- list()
  l2 <- list()
  l3 <- list()
  ...
  for (i in b:end_col) {
    ...
    d <- tibble(eid = prots$eid, raw = prots[[i]]) %>%
      ...
      mutate(smoking = case_when(smoking == -3 ~ NA_integer_,
                                 TRUE ~ smoking),
             across(c(assessment_centre, Batch, ppp_sel, sex, age_recruitment, smoking), as.factor),
             rint_b = rint(raw)) %>%
      select(eid,
             raw,
             rint_b,
