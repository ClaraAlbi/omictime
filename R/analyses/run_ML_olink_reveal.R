...
data_nmr <- readRDS("/mnt/project/biomarkers_3/covariate_res/res_olink.rds") %>%
  select(eid, any_of(tolower(data.table::fread("/mnt/project/Olink Reveal - 2025-05-07.csv")$Gene))) %>%
...
system.time(for (cv in 1:5) {
  ...
  lgb.save(lgb_model, glue("cv.{type}_lightgbm_cv{cv}.txt"))
  ...
  xgb.save(xgb_model, glue("cv.{type}_xgb_cv{cv}.json"))
