...
run_all_models <- function(disease_field) {
  ...
  f_prev1 <- as.formula(paste("prev_case ~", " time_day  + ", paste(base_covars, collapse = " + ")))
  f_prev2 <- update(f_prev1, paste(". ~ . +", paste(extra_covars, collapse = " + ")))

  f_cox1 <- as.formula(paste("Surv(time_yrs, event) ~ ", " time_day  + ",  paste(base_covars, collapse = " + ")))
...
saveRDS(all_results, glue("results_time_diseases.rds"))
