...
data_nmr <- readRDS("/mnt/project/biomarkers_3/covariate_res/res_labs.rds") %>%
...
system.time(for (cv in 1:5) {
  ...
  saveRDS(out_coefs, glue("coefs_labs_cv{cv}.rds"))
  
  for (f in c(glue("cv.{type}_lightgbm_cv{cv}.rds"), glue("cv.{type}_xgb_cv{cv}.rds"), glue("cv.{type}_lasso_cv{cv}.rds"), glue("cv.{type}_lassox2_cv{cv}.rds"))) {
    cmd1 <- paste("dx upload", f, "--destination project-Gp18X60Jk8K49jYjyKqXYY9b:/biomarkers_3/models/")
    system(cmd1)
  }
  for (f in c(glue("predictions_{type}_cv{cv}.rds"), glue("coefs_{type}_cv{cv}.rds"))) {
    cmd2 <- paste("dx upload", f, "--destination project-Gp18X60Jk8K49jYjyKqXYY9b:/biomarkers_3/out/")
    system(cmd2)
  }
