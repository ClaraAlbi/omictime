...
mr_results_chrono <- lapply(
  mr_files[which(get_protein(mr_files) %in% target_prots)],
  run_mr_exposure_to_protein,
  exposure_gwas = gwas_chrono,
  exposure_name = "chrono",
  exposure_beta = "beta_chrono",
  exposure_se   = "se_chrono",
  exposure_eaf  = "AF1_chrono",
  exposure_p    = "P_chrono",
  exposure_n    = "N_chrono",
  exposure_A1   = "A1_chrono",
  exposure_A2   = "A2_chrono"
)

#saveRDS(mr_results_chrono, "results/mr/MR_chrono_proteins.rds")

mr_results_chrono <- readRDS("results/mr/MR_chrono_proteins.rds")
...
p_mrchr <- harm_table %>%
  left_join(mr_table_chrono, by = c("exposure", "outcome")) %>%
  filter(method == "IVW") %>%
  ...
  labs(
    title = "Chronotype -> Protein",
    y = expression("GWAS"~beta~"(Protein)"),
    x = expression("GWAS"~beta~"(Chronotype)"), color = "Included \nin MR"
  ) +
  theme_classic() +
  theme(text = element_text(size = 8))

ggsave("OUTPUTS/MR_chrono_prots.png", p_mrchr, width = 10, height = 8)
