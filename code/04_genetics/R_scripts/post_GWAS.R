...
gwas <- data.table::fread(downloads_path("gwas_res_v2.res.glm.linear")) %>%
  filter(TEST == "ADD")
...
circadian_genes <- data.table::fread(downloads_path("merged_final_collected_CR_geneset_space_removed_annotated_with_gene_name_add_Science_46tissues_baboon_paper_mouse_SCN_related.txt")) %>%
