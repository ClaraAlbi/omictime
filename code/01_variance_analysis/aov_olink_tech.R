...
covs_raw <- readRDS(project_path("biomarkers/covs.rds")) %>%
...
olink_times <- data.table::fread(project_path("Bulk/Protein biomarkers/Olink/helper_files/olink_processing_start_date.dat"))
batch       <- data.table::fread(project_path("Bulk/Protein biomarkers/Olink/helper_files/olink_batch_number.dat"))
panel       <- data.table::fread(project_path("Bulk/Protein biomarkers/Olink/helper_files/olink_assay.dat")) %>%
  mutate(
    prot  = tolower(Assay),
...
metadata <- data.table::fread(project_path("olink_instance_0_meta.csv")) %>%
  rename(
    num_prots = p30900_i0,
    PlateID   = p30901_i0,
    well_id   = p30902_i0,
...
prots <- readRDS(project_path("olink_instance_0_QC.rds"))
...
exclude_terms <- c("sex", "age_recruitment", "bmi", "smoking")
