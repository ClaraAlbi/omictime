...
bioquem_cov <- data.table::fread(project_path("biomarkers_assay_date.tsv")) %>%
  janitor::clean_names()

bioquem <- data.table::fread(project_path("biomarkers_2.tsv")) %>%
  janitor::clean_names()

time <- readRDS(project_path("biomarkers/time.rds"))

covs <- data.table::fread(project_path("circadian/data/covariates.tsv")) %>%
  janitor::clean_names() %>%
...
dilution_factor <- data.table::fread(project_path("dilution_factor.tsv")) %>%
