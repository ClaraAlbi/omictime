...
chrono <- data.table::fread(project_path("chronotype2.tsv")) %>%
...
dat <- data_all %>%
  ...
  inner_join(chrono, by = "eid") %>%
  relocate(eid, chrono)

# feature names once
feat <- setdiff(names(dat), c("eid", "chrono"))
...
system.time({
  for (cv in 1:5) {
    ...
    # y
    y_train <- dat$chrono[idx_train]
    y_test  <- dat$chrono[idx_test]
