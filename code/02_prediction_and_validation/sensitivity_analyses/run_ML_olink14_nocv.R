source("config/paths.R")

library(dplyr)
library(tidyr)
library(glue)
library(Matrix)
library(lightgbm)
library(xgboost)
library(glmnet)

type <- "proteomic_time_prediction_UKB"

data_all <- readRDS("res_olink_14panels.rds")
stopifnot(all(c("eid") %in% names(data_all)))

time <- readRDS(project_path("biomarkers/time.rds")) %>%
  select(eid, time_day) %>%
  filter(time_day > 9 & time_day < 20)

dat <- data_all %>%
  inner_join(time, by = "eid") %>%
  relocate(eid, time_day)

# feature names
feat <- setdiff(names(dat), c("eid", "time_day"))

# ---- helpers
train_scaler <- function(X) {
  mu <- colMeans(X, na.rm = TRUE)
  s  <- suppressWarnings(apply(X, 2, sd, na.rm = TRUE))
  s[!is.finite(s) | s == 0] <- 1
  list(mu = mu, s = s)
}

apply_scaler <- function(X, scaler) {
  for (j in seq_along(scaler$mu)) {
    xj <- X[, j]
    nas <- is.na(xj)
    if (any(nas)) xj[nas] <- scaler$mu[j]
    X[, j] <- (xj - scaler$mu[j]) / scaler$s[j]
  }
  X
}

mk_poly2 <- function(X) {
  X2 <- X ^ 2
  colnames(X2) <- paste0(colnames(X), "_sq")
  cbind(X, X2)
}

# ---- full dataset (no CV)
idx_train <- seq_len(nrow(dat))
idx_test  <- seq_len(nrow(dat))  # in-sample predictions

y_train <- dat$time_day[idx_train]
y_test  <- dat$time_day[idx_test]

X_train_raw <- as.matrix(dat[idx_train, feat, drop = FALSE])
X_test_raw  <- as.matrix(dat[idx_test , feat, drop = FALSE])

# ---- scale/impute
scaler <- train_scaler(X_train_raw)
X_train <- apply_scaler(X_train_raw, scaler)
X_test  <- apply_scaler(X_test_raw , scaler)

# ---- validation split (for early stopping)
set.seed(42)
ntr  <- length(y_train)
nval <- max(1L, floor(0.2 * ntr))
val_ids <- sample(seq_len(ntr), nval)
tr_ids  <- setdiff(seq_len(ntr), val_ids)

# ===================== LIGHTGBM =====================
dtrain_lgb <- lgb.Dataset(X_train[tr_ids, , drop = FALSE], label = y_train[tr_ids])
dvalid_lgb <- lgb.Dataset.create.valid(
  dtrain_lgb,
  data = X_train[val_ids, , drop = FALSE],
  label = y_train[val_ids]
)

lgb_params <- list(
  objective = "regression",
  metric = "l2",
  feature_fraction = 0.8,
  bagging_fraction = 0.8,
  bagging_freq = 5,
  min_data_in_leaf = 50,
  min_sum_hessian_in_leaf = 1,
  lambda_l1 = 5,
  lambda_l2 = 2,
  min_gain_to_split = 8,
  num_threads = max(1L, parallel::detectCores() - 1L)
)

lgb_model <- lgb.train(
  params = lgb_params,
  data = dtrain_lgb,
  valids = list(valid = dvalid_lgb),
  nrounds = 5000,
  early_stopping_rounds = 100,
  verbose = -1
)

lgb.save(lgb_model, as.character(glue("{type}_lightgbm.txt")))

# ===================== XGBOOST ======================
dtrain_xgb <- xgb.DMatrix(X_train, label = y_train)
dvalid_xgb <- xgb.DMatrix(X_train[val_ids, , drop = FALSE], label = y_train[val_ids])

xgb_params <- list(
  objective = "reg:squarederror",
  eval_metric = "rmse",
  eta = 0.05,
  max_depth = 6,
  subsample = 0.8,
  colsample_bytree = 0.8,
  nthread = max(1L, parallel::detectCores() - 1L)
)

xgb_model <- xgb.train(
  params = xgb_params,
  data = dtrain_xgb,
  nrounds = 5000,
  watchlist = list(valid = dvalid_xgb),
  early_stopping_rounds = 100,
  verbose = 0
)

xgb.save(xgb_model, as.character(glue("{type}_xgb.json")))

# ===================== LASSO ========================
lasso_cv <- cv.glmnet(
  x = X_train, y = y_train,
  alpha = 1, family = "gaussian",
  standardize = FALSE,
  nfolds = 5, grouped = TRUE,
  intercept = TRUE
)

saveRDS(lasso_cv, as.character(glue("{type}_lasso.rds")))

# ===================== LASSO (with squares) =========
X_train_x2 <- mk_poly2(X_train)
X_test_x2  <- mk_poly2(X_test)

lasso_x2_cv <- cv.glmnet(
  x = X_train_x2, y = y_train,
  alpha = 1, family = "gaussian",
  standardize = FALSE,
  nfolds = 5, grouped = TRUE,
  intercept = TRUE
)

saveRDS(lasso_x2_cv, as.character(glue("{type}_lassox2.rds")))

# ===================== PREDICT ======================
y_pred_lgb <- predict(lgb_model, X_test)
y_pred_xgb <- predict(xgb_model, xgb.DMatrix(X_test))

y_pred_lasso   <- as.numeric(predict(lasso_cv,   s = "lambda.min", newx = X_test))
y_pred_lassox2 <- as.numeric(predict(lasso_x2_cv, s = "lambda.min", newx = X_test_x2))

out_pred <- tibble(
  eid          = dat$eid,
  time_day     = y_test,
  pred_lgb     = y_pred_lgb,
  pred_xgboost = y_pred_xgb,
  pred_lasso   = y_pred_lasso,
  pred_lassox2 = y_pred_lassox2
)

saveRDS(out_pred, as.character(glue("predictions_{type}.rds")))

# ===================== FEATURE IMPORTANCES ==========
coefs_lasso   <- as.matrix(coef(lasso_cv, s = "lambda.min"))
feats_lasso   <- rownames(coefs_lasso)[coefs_lasso[,1] != 0]
w_lasso       <- coefs_lasso[coefs_lasso[,1] != 0, 1]

coefs_lassox2 <- as.matrix(coef(lasso_x2_cv, s = "lambda.min"))
feats_lassox2 <- rownames(coefs_lassox2)[coefs_lassox2[,1] != 0]
w_lassox2     <- coefs_lassox2[coefs_lassox2[,1] != 0, 1]

imp_xgb <- xgb.importance(model = xgb_model)
imp_lgb <- lgb.importance(lgb_model)

out_coefs <- tibble(
  model   = c(rep("LASSO", length(feats_lasso)),
              rep("LASSOx2", length(feats_lassox2)),
              rep("XGB", nrow(imp_xgb)),
              rep("LGB", nrow(imp_lgb))),
  feature = c(feats_lasso, feats_lassox2, imp_xgb$Feature, imp_lgb$Feature),
  weight  = c(w_lasso, w_lassox2, imp_xgb$Gain, imp_lgb$Gain)
)

saveRDS(out_coefs, as.character(glue("coefs_{type}.rds")))

# ---- quick sanity
cat(glue(
  "R2: LGB={round(cor(y_test, y_pred_lgb)^2,3)}  ",
  "XGB={round(cor(y_test, y_pred_xgb)^2,3)}  ",
  "LASSO={round(cor(y_test, y_pred_lasso)^2,3)}  ",
  "LASSOx2={round(cor(y_test, y_pred_lassox2)^2,3)}\n"
))

gc(FALSE)
