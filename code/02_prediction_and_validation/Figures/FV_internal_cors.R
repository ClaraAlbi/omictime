...
preds_olink <- readRDS(project_path("biomarkers_res/olink_int_replication_v2.rds")) %>%
  filter(i == 0) %>%
...
df <- readRDS(project_path("biomarkers_res/olink_int_replication_v2.rds")) %>%
  filter(!is.na(time_day)) %>%
...
full <- cowplot::plot_grid(part1, final_plot, p_c, nrow = 3, labels = c("", "C", "D"), label_size = 18, rel_heights = c(1, 1, 0.7))
