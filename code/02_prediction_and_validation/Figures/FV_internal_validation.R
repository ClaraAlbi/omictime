...
time_i0 <- readRDS(project_path("biomarkers/time.rds")) %>%
  filter(time_day > 9 & time_day < 20)

###

lgb1 <- lightgbm::lgb.load(project_path("biomarkers_res/cv.olink_14_lightgbm_cv1.lgb"))
xgb <- xgboost::xgb.load(project_path("biomarkers_res/cv.olink_14_xgb_cv1.ubj"))
lasso <- readRDS(project_path("biomarkers_res/cv.olink_14_lasso_cv1.rds"))
lassox2 <- readRDS(project_path("biomarkers_res/cv.olink_14_lassox2_cv1.rds"))

### PREDS i0

l <- c(list.files(project_path("LASSO_exp/"),
...
i2_data <- data.table::fread(project_path("OLINK_i2.tsv")) %>%
...
i2_meta <- data.table::fread(project_path("blood_sampling_instance2.tsv")) %>%
...
# i3
i3_data <- data.table::fread(project_path("OLINK_i3.tsv")) %>%
  select(eid, any_of(lasso$glmnet.fit$beta@Dimnames[[1]])) %>%
  mutate(across(-eid, ~scale(.x)[,1]))
...
i3_meta <- data.table::fread(project_path("blood_sampling_instance3.tsv")) %>%
...
# df <- preds_i0_olink %>% mutate(i = 0) %>%
#   left_join(time_i0 %>% select(eid, date_bsampling)) %>%
#   bind_rows(readRDS(project_path("olink_int_replication.rds")) %>% filter(i!= 0) %>%
#               rowwise() %>% mutate(pred_mean = mean(c(pred_lgb, pred_xgboost, pred_lasso, pred_lassox2))) %>%
#               unnest())
#
...
time_i1 <- data.table::fread(project_path("blood_sampling_instance1.tsv")) %>%
...
nmr_i1 <- data.table::fread(project_path("nmr_nightingale_i1.tsv")) %>%
