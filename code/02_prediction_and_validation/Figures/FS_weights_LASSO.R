...
cv_sets <- readRDS(project_path("LASSO_exp/cv.olink_14panels_subsets.rds"))

data <- readRDS(project_path("LASSO_exp/res_olink_14panels.rds")) %>%
...
compute_cv_score <- function(k, threshold, data, cv_sets) {
  ...
  cv_coef <- readRDS(
    sprintf(project_path("LASSO_exp/coefs_olink_14panels_cv%d.rds"), k)
  ) %>%
    filter(model == "LASSO")
...
p1 <- res %>%
  left_join(readRDS(project_path("biomarkers/time.rds")) %>% select(eid, time_day)) %>%
...
coef_avg <- map_dfr(1:5, ~
                      readRDS(
                        sprintf(project_path("LASSO_exp/coefs_olink_14panels_cv%d.rds"), .x)
                      ) %>%
                      filter(model == "LASSO") %>%
